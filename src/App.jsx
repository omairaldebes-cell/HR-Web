...
import { db, auth, secondaryAuth, googleProvider } from './firebase';
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, onAuthStateChanged, signOut as firebaseSignOut, signInWithPopup } from 'firebase/auth';
...
export default function App() {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      ...
      if (!user && loggedInUser) {
        // Firebase session expired or user was deleted, but local state still thinks logged in
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        if (user.permissions && !user.permissions.includes('dashboard')) {
         ...
         if (loginForm.username === 'faezaldebs@gmail.com') {
             ...
             const docRef = await addDoc(collection(db, 'app_users'), newUserReq);
             setLoggedInUser({ id: docRef.id, ...newUserReq });
             setActiveTab('dashboard');
         } else {
      ...
      if ((err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential' || err.code === 'auth/invalid-login-credentials') && loginForm.username === 'faezaldebs@gmail.com') {
         ...
         try {
           ...
           await addDoc(collection(db, 'app_users'), newUserReq);
           
           alert('تم إنشاء وتهيئة حساب المدير في الخلفية! الرجاء تسجيل الدخول الآن.');
           // We can clear password field or let them login normally
         ...
         console.error(err);
         alert('خطأ في اسم المستخدم أو كلمة المرور: ' + err.code);
      }
    }
  };
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const q = query(collection(db, 'app_users'), where('username', '==', email));
      const snap = await getDocs(q);
      ...
      if (!snap.empty) {
        ...
        if (user.permissions && !user.permissions.includes('dashboard')) {
         ...
         if (email === 'omair.aldebes@gmail.com') { // Auto-provision highest super admin
             const newUserReq = {
                 username: email,
                 role: 'admin',
                 permissions: navItems.map(item => item.id)
             ...
             alert(`تم تسجيل الدخول لكن حسابك (${email}) غير مسجل في النظام كمدير أو موظف.`);
             firebaseSignOut(auth);
         }
      }
    } catch (err) {
      ...
      <div style={{display:'flex', justifyContent:'center', alignItems:'center', height:'100vh', background:'var(--bg-color)'}}>
        <div className="card animate-fade-in" style={{maxWidth: '400px', width: '100%', textAlign: 'center'}}>
          ...
          <button 
            ...
            style={{width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', background: 'var(--surface)'}}>
